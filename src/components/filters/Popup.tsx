...
const Popup: React.FC<PopupProps> = ({
  ...
  const { opened, setOpened, popupRef } = usePopup();

  const handlePopupClick = (obj: OptionObj) => {
    handleClick(obj);
    setOpened(false);
  };

  return (
    <div ref={popupRef} className={style.filter}>
      <div className={style.filter__label}>
        ...
        <b>{title}</b>
        <span onClick={() => setOpened(!opened)}>{activeValue.value}</span>
      </div>
      {opened && (
